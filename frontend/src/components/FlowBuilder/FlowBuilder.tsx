...
export const FlowBuilder = () => {
  ...
  const [outputPreviewTargets, setOutputPreviewTargets] = useState<Record<string, TableTarget[]>>({});
  ...
  */
  ...
  useEffect(() => {
    nodes.forEach((node) => {
      ...
      const destinationTargets = node.data?.destinationTargets as TableTarget[] | undefined;
      if (Array.isArray(destinationTargets) && destinationTargets.length > 0) {
        return;
      }
      const hasBatchSource = sourceTargets.some(
        (target) => typeof target.batchId === 'number' && target.batchId !== null
      );
      if (!hasBatchSource) {
        return;
      }
      const autoDestinations: TableTarget[] = sourceTargets.map((target, index) => {
        const sheetName = target.sheetName ?? 'Sheet1';
        const outputId = `auto:${node.id}:${target.batchId ?? 'single'}:${index}`;
        return {
          fileId: null,
          sheetName,
      ...
      updateNode(node.id, {
        data: {
          ...node.data,
          destination: autoDestinations[0] ?? null,
          destinationTargets: autoDestinations,
        },
      });
    });
  }, [nodes, updateNode]);
    ...
    previewUpdateTimeoutRef.current = setTimeout(() => {
      ...
      (async () => {
        for (const nodeId of uncachedNodes) {
          ...
          const node = nodesSnapshot[index];
          const previousPreview = stepPreviewsRef.current[node.id] ?? null;
          const override = previewOverrides[nodeId];
          const isOutputNode = node.data?.blockType === 'output' || node.type === 'output';
          try {
            if (index === 0) {
              ...
              if (isOutputNode && !override && !outputPreviewTarget) {
                ...
                try {
                  ...
                  setOutputPreviewTargets(prev => ({ ...prev, [node.id]: response.outputs }));

                  if (response.outputs.length === 1) {
                    setPreviewOverrides(prev => ({ ...prev, [node.id]: response.outputs[0] }));
                  } else if (response.outputs.length === 0) {
                    setPreviewErrors(prev => ({ ...prev, [node.id]: "No output generated by the flow." }));
                    setStepPreviews(prev => ({ ...prev, [node.id]: null }));
                  } else {
                    setStepPreviews(prev => ({ ...prev, [node.id]: null }));
                    setPreviewErrors(prev => ({ ...prev, [node.id]: "Multiple outputs available. Please select one to preview." }));
                  }
                } catch (e) {
                  setPreviewErrors(prev => ({ ...prev, [node.id]: "Failed to list available outputs." }));
                } finally {
                  setPreviewLoading(prev => ({ ...prev, [node.id]: false }));
                }
                continue;
              ...
              const previewTarget = override ??
                ...
                (nodeDestination?.fileId || nodeDestination?.virtualId
                  ? nodeDestination
                  : nodeTarget ?? {
                      fileId: fileIdSnapshot ?? sourceFileIds[0] ?? null,
                      sheetName: sheetSnapshot ?? null,
                    });
              const previewTargetIsOutput = Boolean(
                previewTarget?.virtualId?.startsWith('output:')
              );
              const hasSource = Boolean(previewTarget?.fileId || previewTarget?.virtualId);
              if (!hasSource) {
                setStepPreviews((prev) => ({ ...prev, [node.id]: null }));
                setPreviewErrors((prev) => ({
                  ...prev,
              ...
              let previewToStore = result.preview;
              if (
                previewTargetIsOutput &&
                previewToStore.preview_rows.length === 0 &&
                previousPreview &&
                previousPreview.preview_rows.length > 0
              ) {
                previewToStore = previousPreview;
              }
              setStepPreviews((prev) => ({ ...prev, [node.id]: previewToStore }));
              setPreviewErrors((prev) => ({ ...prev, [node.id]: null }));
            }
          } catch (_error) {
            if (previewRunIdRef.current !== runId) {
