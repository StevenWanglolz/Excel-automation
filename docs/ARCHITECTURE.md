...
- `FlowBuilder/` - Sequential pipeline builder with group/single uploads, grouped source/destination presentation, output mapping, and preview selectors that keep batch context when switching source files. Features **Auto-Save on Batch Creation** to prevent data loss in new flows.
